...
export const validationPropertySchema = Yup.object().shape({
  ...
  address: Yup.object().shape({
    ...
    reference: Yup.string().max(
      100,
      "La referencia no debe exceder los 100 caracteres"
    ),
    ...
    city: Yup.string()
      ...
      .matches(
        /^[a-zA-Z\s]*$/,
        "No se permiten caracteres especiales en el nombre de la ciudad"
      ),
  }),
...
export const validationSubPropertySchema = Yup.object().shape({
  ...
  price: Yup.number().when("commonArea", (commonArea, schema) => {
    if (commonArea == "false") {
...
export const validationRolScheme = () => {
  return Yup.object().shape({
