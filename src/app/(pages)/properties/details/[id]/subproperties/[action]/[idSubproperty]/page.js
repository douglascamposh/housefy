...
const Page=({params})=> {
  ...
  const  { data, error, isLoading } = useGetSubPropertiesQuery(String(params.id));
  const [createSale] = useCreateSalePropertyMutation();
  const subPropertiesArray=data ?data:[];
  const action=params.action;
  const subPropertyInfo=subPropertiesArray.find(object=>object.id===params.idSubproperty)
  let Data = useSelector((state) => state.rootReducer.Data);
  const dispatch = useDispatch();
  const handleCreateSales = async (SaleData) => {
    try {
      const response = await createSale(SaleData);
      if (response.data) {
        const successMessage = action === "reserve" ? "Propiedad reservada exitosamente" : "Propiedad vendida exitosamente";
  ...
  const MakeSale=async()=>{
    ...
    DataCopy.status = action === "sale" ? sale_status.sold : sale_status.reserved;
  ...
  const [steps, setSteps] = useState([
    ...
    },
    {
      ...
      content:  <PayForm subPropertyInfo={subPropertyInfo} modifiedPage={modifiedPage} />,
      status: true,
    },
  ]);
  useEffect(() => {
    return()=>{
      dispatch(setData(subPropertySaleScheme));
    }
  }, []);
  useEffect(()=>{
    validateData()
  ...
  useEffect(() => {
    if (subPropertyInfo) {
      setSteps(prevSteps => {
        const updatedSteps = [...prevSteps];
        updatedSteps[1].content = <PayForm subPropertyInfo={subPropertyInfo} modifiedPage={modifiedPage} />;
        return updatedSteps;
      });
    }
  }, [subPropertyInfo]);
  ...
  return (
      ...
      <div className="flex flex-col sm:flex-row h-screen m-[-10px]">
        <div className="w-full sm:w-[55%] sm:ml-10 sm:p-10 text-black sm:overflow-auto">
          ...
          <StepperHorizontal steps={steps} page={page} modifiedPage={modifiedPage}></StepperHorizontal>
          {subPropertyInfo && (
            <div className="mx-4">
              <h1 className="text-2xl sm:text-xl xl:text-2xl font-bold">
              ...
              <div className="flex justify-between mt-3">
                <div>
                  <p className="text-gray-700 font-bold">{subPropertyInfo.code} </p>
        ...
        <div className="bg-gray-100 w-full sm:w-[45%] p-10 flex items-center justify-center">
          <div className="w-full sm:w-[400px]">
            ...
            <h1 className="font-bold text-2xl sm:text-xl mb-2 xl:text-2xl">Resumen</h1>
            {
              Data.customer.name!="" && 
            ...
            <div className="flex justify-between">
              ...
              <p>{Data.onAccount} US$</p>
            </div>
            <div className="h-[1px] bg-gray-400 mt-3 mb-5"></div>
            <div className="flex justify-between font-bold">
              <p className="text-gray-700">Saldo pendiente: </p>
              <p>{subPropertyInfo.price-Data.onAccount} US$</p>
            </div>
            <br />
            <small className="text-gray-400">
              Al completar la venta, aceptas estas Condiciones de uso.
            </small>
            <Button label={action === "reserve" ? "Realizar reserva" : "Realizar venta"} className="w-full" onClick={MakeSale}/>
          </div>
        </div>
      </div>
  </>
